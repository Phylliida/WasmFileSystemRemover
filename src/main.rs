...
// modified from https://github.com/wasm-forge/wasi2ic/blob/main/src/main.rs
use clap::Parser;
...
fn do_wasm_file_processing(args: &arguments::WasmFsRemoverArgs) -> Result<(), anyhow::Error> {
    log::info!(
    ...
    if !args.quiet {
        println!("wasm2fs {}: processing input file: '{}', writing output into '{}'", env!("CARGO_PKG_VERSION"), args.input_file, args.output_file);
    }
...
fn main() -> anyhow::Result<()> {
    env_logger::init();
    let args = arguments::WasmFsRemoverArgs::parse();
